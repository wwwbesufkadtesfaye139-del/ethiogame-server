...
module.exports = function makeAdminRouter(io) {
  ...
  router.get('/stats', async (req, res) => {
    try {
      const [
        ...
        // 🔧 FIX (dead-code cleanup, found during Phase 3 review): this used
        // to query GameSession, a model nothing in the codebase ever writes
        // to — BingoRoom/LudoRoom save every finished game to GameHistory
        // instead. completedGames was always 0 as a result.
        GameHistory.countDocuments({ gameState: 'completed' }),
      ]);

      // Active games are ephemeral, in-memory state (see BingoManager /
      // LudoManager) — a game is only ever written to GameHistory once it
      // finishes, so "active" has no meaning as a database query. Read it
      // from the live managers instead, same as the server:getStats socket
      // handler in userHandlers.js already does.
      const activeSessions =
        (global.bingoManager ? global.bingoManager.getRoomCount() : 0) +
        (global.ludoManager  ? global.ludoManager.getRoomCount()  : 0);
    ...
    try {
      ...
      const [recentTxns, recentGames] = await Promise.all([
        Transaction.find({ telegramId: req.params.telegramId })
          .sort({ createdAt: -1 })
          .limit(10),
        // 🔧 FIX (dead-code cleanup): GameSession is never written to —
        // real completed-game records live in GameHistory instead.
        GameHistory.find({ 'participants.telegramId': req.params.telegramId })
          .sort({ createdAt: -1 })
          .limit(10)
          .select('roomId gameType totalPool winnerPrize winners gameState createdAt'),
      ]);
  ...
  router.get('/sessions', async (req, res) => {
    try {
      ...
      const query = {};
      // 🔧 FIX (dead-code cleanup): GameHistory is the model that's actually
      // populated (GameSession never was — see the note in /stats above).
      // GameHistory only ever records FINISHED games — its gameState is
      // 'completed' | 'cancelled' | 'aborted', with no 'active' state at
      // all, since in-progress games live only in memory until they end
      // (BingoManager/LudoManager). Selecting "Active" in this filter will
      // correctly return no historical rows; the "Active Games" stat card
      // (sourced from the live managers) is where to see what's running
      // right now.
      if (status)   query.gameState = status;
      ...
      // Map GameHistory's own field names to the shape the Sessions tab in
      // AdminScreen.jsx already renders (stakeAmount, players, totalPrizePool,
      // status), so the existing client UI keeps working without changes.
      const mapped = sessions.map(s => ({
        _id:            s._id,
        roomId:         s.roomId,
        gameType:       s.gameType,
        status:         s.gameState,
        stakeAmount:    s.participants?.[0]?.stake ?? null,
        players:        s.participants,
        totalPrizePool: s.totalPool,
        createdAt:      s.createdAt,
      }));

      res.json({ sessions: mapped, total, page, pages: Math.ceil(total / limit) });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });
